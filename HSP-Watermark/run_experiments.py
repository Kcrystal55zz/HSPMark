...
def run_hspmark_experiments(args):
    ...
    print("\n" + "="*80)
    print(f"🌟 RESULTS [HSPMark] (Attack: {args.attack_type.upper()}) 🌟")
...
def run_stealthink_experiments(args):
    ...
    def detect_stealthink(full_text, prompt_text, embedded_message):
        """Run stealthink detection on full_text (prompt + generated) and return bit accuracy."""
        full_ids = tokenizer(full_text, return_tensors="pt").input_ids  # [1, total_len]
        prompt_len_local = tokenizer(prompt_text, return_tensors="pt").input_ids.shape[1]

        gen_len = full_ids.shape[1] - prompt_len_local
        if gen_len < n_gram_len + 1:
            return 0.5  # too short, fallback to chance level
        ...
        for step in range(gen_len):
            if step < n_gram_len:
                continue
            l = prompt_len_local + step - n_gram_len
            r = prompt_len_local + step
            cur_seed = full_ids[:, l:r]
            seed_tuple = tuple(cur_seed[0].tolist())

            reweight_processor._seed_rng(cur_seed)
            bit_pos = torch.randint(
                low=0, high=converted_msg_length, size=(1,),
                generator=reweight_processor.rng
            ).item()
            ...
            if hit is None:
                ...
                vocab_perm = torch.randperm(
                    detector_processor.vocab_size, device='cpu',
        ...
        for pos in range(converted_msg_length):
            ...
            for pos_extracted_msg in msg[pos]:
                ...
                h = hamming_distance(pos_bin_true, pos_bin_extracted)
                correct_bits += chunk_capacity - h
            num_msg = max(1, len(msg[pos]))
            total_correct_bits += correct_bits / num_msg

        return total_correct_bits / msg_len
    ...
    for i, prompt in enumerate(tqdm(prompts, desc=f"Evaluating Stealthink {msg_len}-bits")):
        inputs = tokenizer(prompt, return_tensors="pt").to(device)
        prompt_token_len = inputs.input_ids.size(1)

        # Embed a fresh random message for each sample
        embedded_message = [random.randint(0, num_value - 1) for _ in range(converted_msg_length)]
        bits_for_log = embedded_message[:]

        # [A] Baseline generation (no watermark)
        with torch.no_grad():
        ...
        if is_valid_sample:
            # [C] Clean detection
            acc_clean = detect_stealthink(wm_full, prompt, embedded_message)
            total_acc_clean += acc_clean
            ...
            # [D] Drop attack
            dropped_text = attacker.simulate_drop_attack(wm_text, drop_ratio=args.drop_ratio)
            acc_drop = detect_stealthink(prompt + " " + dropped_text, prompt, embedded_message)
            total_acc_drop += acc_drop
            ...
            if paraphrased_text:
                acc_para = detect_stealthink(prompt + " " + paraphrased_text, prompt, embedded_message)
            total_acc_para += acc_para
    ...
    v = valid_sample_count if valid_sample_count > 0 else 1
    print("\n" + "="*80)
    print(f"🌟 RESULTS [Stealthink] (Attack: {args.attack_type.upper()}) 🌟")
    print(f"有效样本数 / 总样本数: {valid_sample_count} / {len(prompts)}")
    print(f"Clean Bit Acc: {total_acc_clean/v*100:.2f}% | Drop Acc: {total_acc_drop/v*100:.2f}% | Para Acc: {total_acc_para/v*100:.2f}%")
    print(f"Base PPL: {total_ppl_base/v:.2f} | WM PPL: {total_ppl_wm/v:.2f}")
    print("="*80)
...
def main():
    ...
    if args.method == "hspmark":
        run_hspmark_experiments(args)
    elif args.method == "stealthink":
        run_stealthink_experiments(args)
    else:
